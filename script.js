...
// Quran Player Logic
document.addEventListener('DOMContentLoaded', () => {
    const quranBtn = document.getElementById('quran-toggle-btn');
    const quranAudio = document.getElementById('quran-audio');

    if (quranBtn && quranAudio) {
        let isPlaying = false;

        // Handle audio events to sync state in case of buffering/errors
        quranAudio.addEventListener('play', () => {
            quranBtn.classList.add('playing');
            quranBtn.innerHTML = '<i class="fa-solid fa-pause"></i> رکیں (Pause)';
            isPlaying = true;
        });

        quranAudio.addEventListener('pause', () => {
            quranBtn.classList.remove('playing');
            quranBtn.innerHTML = '<i class="fa-solid fa-play"></i> تلاوت قرآن';
            isPlaying = false;
        });
        ...
        quranBtn.addEventListener('click', toggleQuran);
        ...
        const tryAutoplay = () => {
            quranAudio.play().then(() => {
                ...
                // Add one-time click listener to document to start audio
